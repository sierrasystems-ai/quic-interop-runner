...
// RoundTripper performs HTTP/0.9 roundtrips over QUIC or QMux.
type RoundTripper struct {
	mutex sync.Mutex

	TLSClientConfig *tls.Config
	QuicConfig      *quic.Config
	// NextProtos overrides the default ALPN list (hq-interop) when non-empty.
	NextProtos []string
	// Dial, if set, is used instead of quic.DialAddrEarly. Used for QMux over TCP.
...
func (r *RoundTripper) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	if !ok {
		...
		if len(r.NextProtos) > 0 {
			tlsConf.NextProtos = append([]string{}, r.NextProtos...)
		} else {
...
func (c *client) RoundTrip(req *http.Request) (*http.Response, error) {
	c.once.Do(func() {
		if c.dial != nil {
			c.conn, c.dialErr = c.dial(context.Background(), c.hostname, c.tlsConf, c.quicConf)
			return
		}
